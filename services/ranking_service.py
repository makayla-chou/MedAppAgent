...
from scoring.preference_score import (
    DEALBREAKER_STATUS_EXCLUDED,
    DEALBREAKER_STATUS_VERIFY,
...
def resolve_eligibility_status(
    ...
    dealbreaker_status: str,
) -> str:
    """
    Resolve the final four-status eligibility label.

    Confirmed dealbreaker conflicts take precedence over verification.
    """
    statuses = {
        program_status,
        academic_status,
    ...
    if (
        "Not Eligible" in statuses
        or dealbreaker_status == DEALBREAKER_STATUS_EXCLUDED
    ):
        return "Not Eligible"
    ...
    if (
        "Verify Eligibility" in statuses
        or dealbreaker_status == DEALBREAKER_STATUS_VERIFY
    ):
...
def rank_schools(
    ...
    for _, school in schools.iterrows():
        ...
        if dealbreaker_status == DEALBREAKER_STATUS_EXCLUDED:
            eligibility_reasons.extend(
                preference["dealbreaker_conflicts"]
            )
        elif dealbreaker_status == DEALBREAKER_STATUS_VERIFY:
            eligibility_verification.extend(
                preference["dealbreaker_verification"]
            )
